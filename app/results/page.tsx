...
// Helper to generate proprietary winner summary
function generateWinnerSummary(winner: ThumbnailAnalysis, niche: string): string {
  const score = winner.clickScore;
  const fileName = winner.fileName || `Thumbnail ${winner.thumbnailId}`;
  
  if (score >= 80) {
    return `${fileName} is the best choice for your audience. It pairs a clear focal point with readable text and strong visual contrast, mirroring patterns seen in high-performing videos for this niche.`;
  } else if (score >= 65) {
    return `${fileName} offers solid performance potential. The visual composition demonstrates good fundamentals with room for optimization to maximize click-through rates in the ${niche} category.`;
  } else {
    return `${fileName} shows potential but needs refinement. Focus on improving visual clarity and subject prominence to better align with successful ${niche} content patterns.`;
  }
}
...
function ResultsContent() {
    ...
    async function fetchAnalysis() {
      if (!sessionId) {
        ...
        try {
          ...
          // Get image URLs for display with error handling
          let storedImageUrls: string[] = [];
          try {
            const imageUrlsJson = sessionStorage.getItem('imageUrls');
            storedImageUrls = imageUrlsJson ? JSON.parse(imageUrlsJson) : [];
            setImageUrls(storedImageUrls);
          } catch (error) {
            console.warn('Failed to load stored image URLs, using placeholder images:', error);
            setImageUrls([]); // Fallback to empty array
          }
          
          // Use stored base64 data URLs directly for API
          const thumbnailsWithData = thumbnails.map((thumb: any, index: number) => {
            if (storedImageUrls[index]) {
              return {
                ...thumb,
                dataUrl: storedImageUrls[index] // Already base64 data URL
              };
            }
            return thumb;
          });
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-[#0a0f25] via-[#0d1229] to-[#0a0f25] text-white p-8 md:p-24">
            ...
            <div className="bg-gradient-to-br from-purple-900/50 to-blue-900/50 border border-purple-600/20 rounded-2xl p-8 text-center">
              <div className="mb-6">
                ...
                <p className="text-lg text-gray-300 mb-6">
                  You've experienced Thumbscore's power. Unlock unlimited analyses and advanced features.
                </p>
              </div>
                    ...
                    {winner.gptSummary && winner.gptSummary.winner_summary && (
                      <div className="bg-gradient-to-r from-blue-900/40 to-purple-900/40 rounded-lg p-4 mb-6 border border-blue-500/30">
                        ...
                        <div className="text-gray-200 text-base leading-relaxed">
                          {winner.gptSummary.winner_summary}
                </div>
              </div>
                    )}
